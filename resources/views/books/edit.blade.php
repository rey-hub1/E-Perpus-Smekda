...
<div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg border-t-4 border-cta">
    ...
    <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div class="flex flex-col gap-4">
            <div class="flex-1 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                ...
                <div id="cropperContainer" class="hidden mt-4">
                    <p class="text-sm font-bold text-primary mb-2">Sesuaikan Posisi Cover Baru (640x853):</p>
            ...
            @if ($book->gambar)
                <div class="w-32 p-3 bg-white rounded-lg shadow-sm border border-gray-100 self-start" id="currentCoverContainer">
                    <p class="text-[10px] uppercase tracking-wider font-bold text-gray-400 mb-2 text-center">Cover Saat Ini</p>
                    <img src="{{ $book->cover_url }}" class="w-full h-auto rounded shadow-sm">
                </div>
            @endif
        </div>
