...
<body class="flex h-screen overflow-hidden bg-background">
    ...
    <aside class="w-60 shrink-0 flex flex-col bg-white border-r border-gray-200">
        ...
        <div class="px-6 py-5 border-b border-gray-100 flex items-center gap-3">
            ...
            <div>
                <p class="text-sm font-bold text-text leading-none">GoRead</p>
                <p class="text-[10px] text-text/40 uppercase tracking-widest mt-0.5">Admin Panel</p>
            </div>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            <a href="{{ route('admin.dashboard') }}"
                ...
                {{ request()->routeIs('admin.dashboard') ? 'bg-primary text-background' : 'text-text/60 hover:bg-text/5 hover:text-text' }}">
            ...
            <a href="{{ route('admin.books.index') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                {{ request()->routeIs('admin.books.*') ? 'bg-primary text-background' : 'text-text/60 hover:bg-text/5 hover:text-text' }}">
                <svg class="w-4.5 h-4.5 shrink-0" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.categories.index') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                {{ request()->routeIs('admin.categories.*') ? 'bg-primary text-background' : 'text-text/60 hover:bg-text/5 hover:text-text' }}">
                <svg class="w-4.5 h-4.5 shrink-0" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.transactions') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                {{ request()->routeIs('admin.transactions') ? 'bg-primary text-background' : 'text-text/60 hover:bg-text/5 hover:text-text' }}">
                <svg class="w-4.5 h-4.5 shrink-0" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <a href="{{ route('admin.users.index') }}"
                class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                {{ request()->routeIs('admin.users.*') ? 'bg-primary text-background' : 'text-text/60 hover:bg-text/5 hover:text-text' }}">
                <svg class="w-4.5 h-4.5 shrink-0" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="px-3 py-3 border-t border-gray-100 space-y-1">
            <div class="px-3 py-2 flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-xs font-semibold text-text truncate">{{ Auth::user()->name ?? 'Admin' }}</p>
                    <p class="text-[10px] text-text/40 truncate">Administrator</p>
                </div>
            </div>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit"
                    class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-text/50 hover:bg-primary/5 hover:text-primary transition-colors">
