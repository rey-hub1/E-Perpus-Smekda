...
<div class="bg-white p-8 rounded-xl shadow-lg border-t-4 border-primary">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('books.index') }}" class="bg-gray-100 text-primary px-4 py-2 rounded-lg hover:bg-gray-200 transition font-semibold">
            &larr; Kelola Buku
        </a>
    </div>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200">
        <table class="w-full text-left">
            <thead class="bg-primary text-white">
                <tr>
                    <th class="p-4">No</th>
                    <th class="p-4">Peminjam</th> <th class="p-4">Buku</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="p-4 text-center">
                        @if($trx->status == 'dipinjam')
                            <form action="{{ route('admin.return', $trx->id) }}" method="POST" onsubmit="return confirm('Yakin mau memproses pengembalian buku ini secara manual?')">
                                @csrf
                                <button type="submit" class="bg-secondary text-white px-3 py-1 rounded text-xs font-bold hover:bg-primary transition shadow" title="Proses Pengembalian Manual">
                                    ✅ Selesai
                                </button>
                            </form>
                        @else
